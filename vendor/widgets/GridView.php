...
class GridView
{
    ...
    public function __construct($data = [])
    {
        ...
        $this->showLink = $data['showRowLink'];
        $this->data = $data;
    ...
    {
        if ($this->showLink != '') {
            $this->columnName[] = 'id as "showRowLink"';
        }
        if (empty($_GET)) {
    ...
    private function setLabels()
    {
        ...
        if($this->showLink !== null) {
            $this->tableLabel .= '<th>view item</th>';
        }
        $this->tableLabel .= '</tr>';
    ...
    private function setBody()
    {
        foreach ($this->rows as $row) {
            ...
            foreach ($row as $key => $item) {
                ...
                if($key === 'SHOWROWLINK') {
                    $this->bodyRows .= '<td>' . '<a href="/' . $this->searchModel->getTableName() . "/" . $this->showLink . '?id=' . $item . '">view</a>' . '</td>';
